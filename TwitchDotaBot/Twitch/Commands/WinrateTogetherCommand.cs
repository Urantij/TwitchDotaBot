...
public class WinrateTogetherCommand : BaseCommand
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // нужно найти матчи, в которых стример и указанный игрок в одной команде
        ...
        if (wins == 0 && loses == 0)
        {
            await chatbot.Channel.SendMessageAsync($"Вместестояния с {hero.LocalizedName} не найдены.", e.id);
            return;
        }
        ...
        await chatbot.Channel.SendMessageAsync($"Вместе с {hero.LocalizedName} {winrate:F2}% ({wins}/{loses})", e.id);
    }
}